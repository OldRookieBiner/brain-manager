...
export class KnowledgeRetriever {
  ...
  async search(query, limit = 5, options = {}) {
    ...
    // 检查缓存
    const cacheKey = `${query}-${category}-${notebook}-${searchLocal}-${searchSiYuan}`;
    if (useCache) {
  ...
  async searchSiYuanInNotebook(query, notebookName, category = null) {
    ...
    try {
      // 使用思源笔记全文搜索 API（性能优化）
      const response = await fetch(`${this.siYuanApiUrl}/api/search/fullTextSearchBlock`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          paths: category ? [`${notebookName}/${category}`] : [notebookName],
          groupBy: 0,
          orderBy: 0
        })
      });
      ...
      if (data.code === 0 && data.data && data.data.blocks) {
        for (const block of data.data.blocks) {
          ...
          results.push({
            id: block.id,
            title: this.extractTitle(content) || block.content.substring(0, 50),
            category: category || '思源笔记',
            excerpt: this.truncate(content, 200),
            relevance: block.score || 10,
            source: 'siyuan',
