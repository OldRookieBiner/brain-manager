...
export class SiYuanSync {
   ...
   * @param {string} customPath - 自定义路径（可选，支持多层级）
   * @returns {Promise<Object>} 同步结果
   */
  async syncToNotebook(knowledgeCard, customPath = null) {
    const { title, category, content, tags, createdAt } = knowledgeCard;

    try {
      console.log('[SiYuan] 开始同步知识卡片...');
      console.log('[SiYuan] 标题:', title);
      console.log('[SiYuan] 分类:', category);
      console.log('[SiYuan] 内容长度:', content.length, '字符');
      ...
      if (!this.notebookId) {
        ...
        console.log('[SiYuan] 笔记本 ID:', this.notebookId);
      }

      // 2. 创建或更新文档（支持多层级路径）
      const docPath = customPath || `/${category}/${title}`;
      console.log('[SiYuan] 文档路径:', docPath);
      
      const docId = await this.createOrUpdateDoc(docPath, content);
      console.log('[SiYuan] 文档 ID:', docId);

      // 3. 验证文档内容
      const verifyResult = await this.verifyDocument(docId, content);
      if (!verifyResult.success) {
        console.error('[SiYuan] 文档验证失败:', verifyResult.error);
        throw new Error(`文档写入验证失败：${verifyResult.error}`);
      }
      console.log('[SiYuan] 文档验证成功');

      // 4. 设置文档属性（标签等）
      if (tags && tags.length > 0) {
        await this.setDocAttributes(docId, { tags: tags.join(','), category, createdAt });
        console.log('[SiYuan] 文档属性设置完成');
      }

      // 5. 本地备份
      await this.saveLocalBackup(knowledgeCard);
      console.log('[SiYuan] 本地备份完成');
      ...
      console.error('[SiYuan] 同步失败:', error);
  ...
  async createOrUpdateDoc(docPath, content) {
    console.log('[SiYuan] 创建/更新文档:', docPath);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('[SiYuan] 创建文档 HTTP 错误:', response.status, errorText);
        throw new Error(`创建文档失败：${response.status} - ${errorText}`);
      }

      const data = await response.json();
      console.log('[SiYuan] API 响应:', data);
      
      if (data.code === 0) {
        console.log('[SiYuan] 文档创建成功，ID:', data.data.id);
        return data.data.id;
      } else if (data.code === 404 || data.msg?.includes('exists') || data.msg?.includes('已存在')) {
        // 文档已存在，尝试更新
        console.log('[SiYuan] 文档已存在，尝试更新...');
        return await this.updateDoc(normalizedPath, content);
      } else {
        console.error('[SiYuan] 创建文档失败:', data.msg);
        throw new Error(`创建文档失败：${data.msg}`);
      }
    } catch (error) {
      console.error('[SiYuan] createOrUpdateDoc 异常:', error);
      
      // 如果创建失败，尝试更新
      if (error.message.includes('已存在') || error.message.includes('exists')) {
        console.log('[SiYuan] 检测到文档已存在，尝试更新...');
   ...
   * 验证文档内容是否正确写入
   */
  async verifyDocument(docId, expectedContent) {
    try {
      console.log('[SiYuan] 验证文档内容，ID:', docId);
      ...
      if (data.code === 0 && data.data) {
        ...
        // 检查内容长度（允许一定差异）
        const lengthDiff = Math.abs(actualContent.length - expectedContent.length);
        const lengthRatio = lengthDiff / expectedContent.length;
        
        if (lengthRatio > 0.5) {
          console.warn('[SiYuan] 内容长度差异较大:', {
        ...
        console.log('[SiYuan] 文档验证通过，内容长度:', actualContent.length);
        ...
        return {
          success: false,
          error: data.msg || '获取文档内容失败'
        };
      }
    } catch (error) {
      console.error('[SiYuan] 验证文档失败:', error);
      return {
        success: false,
        error: error.message
      };
